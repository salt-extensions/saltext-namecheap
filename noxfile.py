...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/namecheap/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/namecheap/*")
            ...
            #    "--include=src/saltext/namecheap/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/namecheap/config/schemas",
    )
