...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/saltext_namecheap/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/saltext_namecheap/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/saltext_namecheap/*"
            )
            ...
            #    "--include=src/saltext/saltext_namecheap/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/saltext_namecheap/config/schemas",
    )
